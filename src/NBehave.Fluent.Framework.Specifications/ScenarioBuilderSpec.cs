...
namespace NBehave.Fluent.Framework.Specifications
{
    ...
    public abstract class ScenarioBuilderSpec
    {
        ...
        [TestFixture]
        public class When_there_are_tags : ScenarioBuilderSpec
        {
            private static bool beforeScenario;
            private static bool afterScenario;
            private static bool beforeStep;
            private static bool afterStep;

            [Hooks]
            public class MyHooks
            {
                [BeforeScenario]
                public Action BeforeScenario = () => { beforeScenario = true; };
                [AfterScenario]
                public Action AfterScenario = () => { afterScenario = true; };
                [BeforeStep]
                public Action BeforeStep = () => { beforeStep = true; };
                [AfterStep]
                public Action AfterStep = () => { afterStep = true; };
            }

            [Test]
            public void Should_call_BeforeScenario()
            {
                beforeScenario = false;
                bool beforeScenarioValueInStep = false;
                new Feature("Hello")
                    .AddScenario()
                    .Given("something to call", () =>
                        {
                            Assert.IsTrue(beforeScenario, "BeforeScenario was not invoked");
                            beforeScenarioValueInStep = beforeScenario;
                        });
                Assert.IsTrue(beforeScenario, "BeforeScenario was not invoked");
                Assert.IsTrue(beforeScenarioValueInStep, "BeforeScenario was not invoked");
            }
            ...
            public void Should_call_AfterScenario()
            {
                ...
                new Feature("Hello")
                    .AddScenario()
                    .Given("something to call", () => {  })
                    .And("Something", () => Assert.IsFalse(afterScenario, "AfterScenario was called to early"));
                Assert.IsTrue(afterScenario, "AfterScenario was not invoked");
            }

            [Test]
            public void Should_call_before_step()
            {
                beforeStep = false;
                new Feature("Hello")
                    .AddScenario()
                    .Given("something to call", () => Assert.IsTrue(beforeStep, "BeforeStep was not invoked"));
            }

            [Test]
            public void Should_call_after_step()
            {
                afterStep = false;
                new Feature("Hello")
                    .AddScenario()
                    .Given("something to call", () => Assert.IsFalse(afterStep, "AfterStep was invoked"));
                Assert.IsTrue(afterStep, "AfterStep was not invoked");
            }
        }
    }
}
