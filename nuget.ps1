...
task NuGet-Fluent -depends Clean -precondition{ return $frameworkVersion -eq "4.0" } {
	Build-FluentPackage "NUnit" "NUnit.Framework" @( @("nunit", (nunitVersion)), @("", "") )
	Build-FluentPackage "MbUnit" "MbUnit.Framework" @( @("MbUnit", (mbunitVersion)), @("", "") )
	Build-FluentPackage "Xunit" "Xunit.Framework" @( @("Xunit", (xunitVersion)), @("xunit.extensions", (xunitVersion)) )
...
task NuGet-Spec -depends Clean -precondition{ return $frameworkVersion -eq "4.0" } {
	...
	$mb = mbunitVersion
	$x = xunitVersion
	Build-SpecPackage "NUnit" "NUnit.Framework" @( @("nunit", (nunitVersion)), @("", "") )
	Build-SpecPackage "MbUnit" "MbUnit.Framework" @( @("mbunit", (mbunitVersion)),   @("", "") )
	Build-SpecPackage "Xunit" "Xunit" @( @("xunit", (xunitVersion)) ,  @("xunit.extensions", (xunitVersion)) )
